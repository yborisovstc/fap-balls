...
struct CF_Rect
{
	CF_Rect(float aLeft, float aTop, float aRight, float aBottom) {iLeftUpper.iX=aLeft; iLeftUpper.iY=aTop; iRightLower.iX=aRight; iRightLower.iY=aBottom;}
	CF_Rect() {}
	CF_TdPointF iLeftUpper;
	CF_TdPointF iRightLower;
};
...
public:
	...
	void AddBallL(CFT_Ball* aObBall);
protected:
...
public:
	...
	CAE_TState<CF_TdPoint>*	iMcPos; // Current mouse cursor position
//	CAE_TState<HWND>*	iHwnd;
private:
	CAE_State*	iBeaconC;
	CAE_State*	iBeaconX;
...
private:
	...
	template <class T> inline static T GetInp(CAE_StateBase* aState, TInt aInd, const char* aName, T& aVal)
	{
		...
		CAE_TState<T> *tstate = NULL;
		tstate = state->GetObject(tstate);
